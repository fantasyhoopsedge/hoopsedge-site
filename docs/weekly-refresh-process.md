...
**This bites `depth-chart:pipeline` too, not just the standalone projection stages** — found on the second real CI run 2026-09-04. Its last step, `flag_role_changes.py`, reads `output/foundation/*.parquet` directly with no existence check, so `weekly-roster-refresh.yml` (Phase A) also needs a Stage 0 build step even though it never touches the projection stages at all. `prep_depth_chart.py`, the step right before it, reads `output/season-projections-2026-27.json` too but guards that read with an existence check (falls back to seeding blank tiers) — which is exactly why it succeeded on the first fix attempt and made this second gap easy to miss. If you ever add a new consumer of `models/`'s stage outputs, check whether it hard-reads a gitignored `output/` path before assuming "it worked locally" means it'll work in CI.
