...
9. **No validation gate** — `partially closed`. Unlike `seasonal:build`, which is "validation-gated" against a reference export per `CLAUDE.md`, nothing checks a freshly merged dynasty file for null positions/ages, duplicate players, or wildly discontinuous ranks before it ships — those have all been real bugs in past refreshes (commits `28f9dae`, `63063ae`). The July 2026 Dizzle merge (§12) did this by hand each time (typecheck, position-enum check, zero-rank-orphan check, sequential-consensusRank check) — worth codifying into a real script-level gate rather than repeating manually per source. **2026-09-05:** one specific slice of this now has a real gate — `npm run dynasty:verify-rookies` (`scripts/verify-dynasty-rookie-flags.ts`), wired as the first step of `dynasty:sync`. It cross-checks every row's `isRookie` against `nba_roster`'s own `yos` column (ecosystem is the source of truth, per §5) and fails loudly on a mismatch. Built after finding 11 rows shipped with `isRookie: false` despite `nba_roster` marking them `yos: "R"` — all 2026 draftees folded in during the "direct entry" phase (item 6) with the rookie flag simply never set: Emanuel Sharp, Tyler Bilodeau, Bogoljub Markovic, Izaiyah Nelson, Tobi Lawal, Vsevolod Ishchenko, Michael Ajayi, Felix Okpara, Tarik Biberovic, Tyler Nickel, Aaron Nkrumah. The user-visible symptom was two-fold, both driven by the same flag via `playerHeadshotUrl()`/tier-view/rankings-table in `src/lib/dynasty-rankings.ts`: no rookie badge, and the wrong headshot branch (cdn.nba.com lookup instead of the local `/images/prospects/*.jpg` a rookie should use). All 11 fixed by hand this session; the general "no unified ingest script" gap (item 1) remains open for everything this specific check doesn't cover (positions, ages, duplicate rows, discontinuous ranks).

    **Thomas Sorber was in the original 12 and turned out to be a false positive, caught by the owner the same day.** Drafted 2025 (pick 15, OKC), he's been hurt and has yet to make his NBA debut — but he spent 2025-26 on the Thunder's roster, not stashed elsewhere, so 2026-27 is his sophomore season of *service* despite still being his rookie season on the floor. His `isRookie: false` was right all along; the real bug was one row upstream in `data/nba-rosters/2026-27.csv`, which still had his `yos` at `"R"` instead of rolled to `"1"` for the new season — so the new check's own ground truth was wrong for exactly his row. Fixed at the source (`yos` "R" → "1", re-ingested via `roster_ingest.ts --team OKC`, confirmed live: `nba_roster.is_sophomore` now true, `is_incoming_rookie` false, `contract_status` unaffected at "Rookie Scale"). Distinguishing him from a real rookie required more than `yos` alone — see §9's note in `verify-dynasty-rookie-flags.ts` on why `roster_ingest.ts`'s own Markovic-era conflict check (`tagConflicts`) doesn't catch a player whose `prior_team` never changed. Cross-referencing every `yos: "R"` row against an actual NBA franchise name in `prior_team` (not just "any prior_team present") is what surfaced him — he was the only leaguewide match — and is worth re-running after every roster refresh until that check is folded into `roster_ingest.ts` itself.

    **Not a bug, checked and ruled out:** Emanuel Sharp's `nba_roster.contract_status` of "Standard" (surfaced on `/team-rosters` and in `real_salary_values`) is correct, not drift. `roster_ingest.ts`'s `deriveStatus()` only assigns "Rookie Scale" to first-round picks (`draftPick <= 30`) — that mirrors actual NBA CBA rules, since the fixed rookie-scale contract structure is legally exclusive to first-rounders. Sharp is pick 45 (2nd round); his contract is a real "Standard" deal signed via the second-round-pick cap exception (confirmed by his own listed "second-round-exception" signing method). Don't change this heuristic to fold 2nd-rounders into "Rookie Scale" — it would misclassify every real 2nd-round rookie deal on the roster, not just fix Sharp's row.
