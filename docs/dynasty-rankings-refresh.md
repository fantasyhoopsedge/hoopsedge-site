...
- **Prepared:** 2026-07-14 · **Last updated:** 2026-07-15
- **Covers:** `src/lib/dynasty-rankings.{ts,json}`, `/dynasty-rankings`
- **Current dataset:** v1.1 (July 2026) · 496 players — all four updating experts (Dizzle, Dynatyze, MBall, Hashtag) merged, Angle carried forward unchanged from May 2026. Version bump complete; `trend`/`trendDelta` are live for the first time, across the consensus page and the rest of the ecosystem. **Angle's eventual update still needs to be folded into this same v1.1 — see §12, do not bump to v1.2 for it.** See §12 for the full log of this refresh.
...
4. Now that a real expert-vs-ecosystem team conflict has come up and turned out to be the expert being *right* (§12) — should the skill always flag these for the owner, or is there a confidence signal (e.g. a very recent CSV date, a specific transaction note) that would let it auto-resolve toward the expert instead of the ecosystem in some cases?

## 12. July 2026 refresh log

The first refresh actually run against this spec, in progress as of 2026-07-15 (started same day the spec above was written). Four of five experts are updating this cycle — **Angle has not published since May 2026** and is intentionally carried forward unchanged, not dropped or re-derived.
...
**IMPORTANT — standing exception to the "every refresh bumps the version" rule (§10 item 8, §1):** Angle has not published since May 2026 and is carried forward unchanged in this v1.1. When Angle's next list does arrive, it gets merged into **this same v1.1** (re-run the snapshot script with `--version 1.1` again, overwriting `v1.1.json` and updating `expertDates.angle`) — it does **not** trigger a v1.2. v1.2 is reserved for the *next* full periodic refresh cycle, not for Angle catching up within this one. The trend field (below) also needs recomputing at that point, since it will change once avgRank includes a real Angle contribution instead of May's carried-forward numbers.
...
*Sources: `src/lib/dynasty-rankings.{ts,json}` · `src/app/dynasty-rankings/**` · `scripts/swap-experts.js` · `scripts/snapshot-rankings.js` · `scripts/nba-data/roster_ingest.ts` · `dynasty ranks/*.csv` · `dynasty ranks/July 2026/*.csv` · `data/nba-rosters/2026-27.csv` · `data/fhe_2026_prospects_master.csv` · git log (commits `d19246d` through `74d0a88`)*
