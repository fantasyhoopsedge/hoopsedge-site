...
import { RookieBoardEditor } from "./_editor";
...
export const metadata: Metadata = {
  title: "Rookie Board Editor · FHE Admin",
  robots: { index: false, follow: false },
};

export default async function RookieBoardAdminPage() {
  if (process.env.NODE_ENV !== "production") {
    return <RookieBoardEditor />; // localhost is trusted
  }

  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/prediction-arena?next=/admin/rookie-board");
  ...
  if (!(await isRbAdmin(user.email))) {
    return (
      <main style={{ padding: "80px 32px", textAlign: "center", color: "#94a3b8", fontFamily: "system-ui" }}>
        ...
        <p>Your account isn&apos;t an authorized rookie-board editor.</p>
      </main>
    );
  }

  return <RookieBoardEditor />;
}
